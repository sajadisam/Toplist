...
import get_server from "@/backend/server";

export default async function Page({ params }: { params: { server: string } }) {
  const data = (await get_server(decodeURIComponent(params.server)))!;
  return <>
    <div className="pattern py-0">
      ...
      <BodyWrapper>
        <div className="flex flex-col 2md:flex-row  gap-7">
          <LeftSidebar image={data.image} name={data.name} description={data.description} banner={data.banner} />
          <RightSidebar votes={data.vote[0].vote_count} added={data.created_at} updated={data.updated_at} />
        </div>
        {/* <div> */}
        {/*   <ServerTabs /> */}
        {/*   <ServerReview /> */}
    ...
    <section className="flex-[7] bg-black bg-opacity-10 px-4 py-2 rounded-sm">
      <div className="flex items-center justify-between flex-wrap">
        ...
        <div className="flex gap-3">
          ...
          <button><FontAwesomeIcon icon={faWarning} className="text-red-600" width={20} height={20} /></button>
        </div>
      </div>
...
function RightSidebar(props: RightSidebarProps) {
  const categories = [
    ...
    // },
    {
      ...
      value: (props.added).toLocaleDateString('en-us', { year: "numeric", month: "short", day: "numeric" }),
      tooltip: "Server type",
    },
    {
      icon: faRefresh,
      value: (props.updated).toLocaleDateString('en-us', { year: "numeric", month: "short", day: "numeric" }),
      tooltip: "Server type",
    },
  ];
  ...
  return <>
    <section className="flex-[2] ">
      <DataTable categories={categories.filter((item) => item.value)} />
    </section>
  </>;
}
      ...
      <div className="flex flex-col flex-1 gap-2 mt-3">
        <div className="flex gap-0.5 items-center">
          {(Array(5).fill(0).map((_, index) => (
            <FontAwesomeIcon key={index} icon={faStar} className={`${index < Math.round(average) ? "text-yellow-500" : "text-gray-700"}`} width={20} height={20} />
          )))}
